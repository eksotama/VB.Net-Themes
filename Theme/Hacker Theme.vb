...
Class Credits
    'Coded and designed by H?WK

    'Credits to Aeonhack for the great themebase.
    'His free themebase 1.5.4 is available on patebin.com at: 
    'Be sure to give credits if you use it.

    'Credits to Mavamaarten for allowing me to use a few controls from his Ghost theme as a base for mine.
    'Without looking at his controls and their functions, creating this theme wouldn't be possible.
    'I also got the idea for the seperate title box from Mavamaarten~'s Divine theme

    'Credits to Support™ for his permission to redesign his ON/OFF checkbox and radiobutton from his V Theme.
    'V Theme link: 
End Class
...
Class HackerTheme
    ...
    Protected Overrides Sub PaintHook()
        ...
        'Form Border
        Dim HB As New HatchBrush(HatchStyle.DarkDownwardDiagonal, Color.FromArgb(255, 16, 16, 16), Color.FromArgb(255, 14, 14, 14))
        G.FillRectangle(HB, New Rectangle(0, 0, Width - 1, Height - 1))
        G.DrawRectangle(Border, 0, 0, Width - 1, Height - 1)
        'Form Interior
        Dim HB2 As New HatchBrush(HatchStyle.DarkDownwardDiagonal, Color.FromArgb(120, 80, 80, 80), Color.FromArgb(120, 64, 64, 64))
        G.FillRectangle(HB2, New Rectangle(6, 25, Width - 11, Height - 30))
        'Title Box
        Dim HB3 As New HatchBrush(HatchStyle.DarkDownwardDiagonal, Color.FromArgb(255, 20, 20, 20), Color.FromArgb(255, 12, 12, 12))
        Dim p() As Point = {New Point(3, 15), New Point(20, 3), New Point(230, 3), New Point(230, 15), New Point(200, 35), New Point(3, 35)}
        G.FillPolygon(HB3, p)
        G.DrawPolygon(Pens.Black, p)
        'Icon and Form Title
        G.DrawString(Text, Font, TextBrush, New Point(40, 12))
        G.DrawIcon(FindForm.Icon, New Rectangle(20, 12, 16, 16))
        'Draw Border
    ...
    Protected Overrides Sub PaintHook()
        ...
        Select Case State
            Case MouseState.None
                Dim LGB As New LinearGradientBrush(New Rectangle(1, 1, Width - 2, Height - 2), Color.FromArgb(255, 32, 32, 32), Color.Black, 75.0F)
                G.FillRectangle(LGB, New Rectangle(1, 1, Width - 2, Height - 2))
                DrawText(TextColor, HorizontalAlignment.Center, 0, 0)
            Case MouseState.Over
                Dim LGB As New LinearGradientBrush(New Rectangle(1, 1, Width - 2, Height - 2), Color.FromArgb(255, 20, 20, 20), Color.Black, 75.0F)
                G.FillRectangle(LGB, New Rectangle(1, 1, Width - 2, Height - 2))
                DrawText(TextColor, HorizontalAlignment.Center, 0, 0)
            Case MouseState.Down
                Dim LGB As New LinearGradientBrush(New Rectangle(1, 1, Width - 2, Height - 2), Color.FromArgb(255, 12, 12, 12), Color.Black, 75.0F)
                G.FillRectangle(LGB, New Rectangle(1, 1, Width - 2, Height - 2))
                DrawText(TextColor, HorizontalAlignment.Center, 0, 0)
        End Select
...
Class HackerGroupBox
    ...
    Protected Overrides Sub PaintHook()
        G.Clear(Color.Black)
        'Group Area
        Dim BackBG As New HatchBrush(HatchStyle.DarkDownwardDiagonal, Color.FromArgb(255, 16, 16, 16), Color.FromArgb(255, 14, 14, 14))
        G.FillRectangle(BackBG, New Rectangle(0, 22, Width - 1, Height - 23))
        'Text Area
        Dim TopBG As New LinearGradientBrush(New Rectangle(0, 0, 75, 22), Color.FromArgb(255, 18, 18, 18), Color.FromArgb(255, Color.Black), 180.0F)
...
#End Region
    ...
    Sub New()
        ...
        SetColor("Border1", Color.FromArgb(255, 24, 24, 24))
        SetColor("Border2", Color.FromArgb(255, 12, 12, 12))
        Font = New Font("Arial", 9)
    End Sub
...
#End Region
    ...
    Protected Overrides Sub PaintHook()
        ...
        If (_Checked = False) Then
            ...
            Dim offLGB As New LinearGradientBrush(offRect, Color.FromArgb(150, Color.DarkRed), Color.DarkRed, 90.0F)
        ...
        Else
            ...
            Dim onLGB As New LinearGradientBrush(onRect, Color.FromArgb(150, Color.Gainsboro), Color.LightGray, 90.0F)
...
#Region "Events"
    ...
    Protected Overrides Sub OnClick(ByVal e As System.EventArgs)
        ...
        For Each C As Control In Parent.Controls
            If C.GetType.ToString = Replace(My.Application.Info.ProductName, " ", "_") & ".HackerRadioButton" Then
                Dim CC As HackerRadioButton
                CC = C
                CC.Checked = False
            End If
        Next
...
#End Region
    ...
    Protected Overrides Sub PaintHook()
        ...
        'Text Area
        Dim textAreaHB As New HatchBrush(HatchStyle.DarkDownwardDiagonal, Color.FromArgb(200, 16, 16, 16), Color.FromArgb(200, 14, 14, 14))
        Dim textAreaRect As New Rectangle(8, 0, Width - 9, Height - 1)
        G.FillRectangle(textAreaHB, textAreaRect)
        ...
        Else
            G.FillEllipse(New SolidBrush(Color.Black), 0, 0, 16, 16)
            Dim LGB1 As New LinearGradientBrush(New Rectangle(1, 1, 14, 14), Color.FromArgb(24, 24, 24), Color.FromArgb(10, 10, 10), 90.0F)
            G.FillEllipse(LGB1, New Rectangle(1, 1, 14, 14))
            Dim LGB2 As New LinearGradientBrush(New Rectangle(2, 2, 12, 12), Color.FromArgb(16, 16, 16), Color.FromArgb(8, 8, 8), 90.0F)
            ...
            G.DrawEllipse(Pens.DarkGreen, 4, 4, 8, 8)
            G.FillEllipse(SB, New Rectangle(4, 4, 8, 8))
            G.DrawEllipse(Pens.White, 0, 0, 16, 16)
        End If
        'Text
        G.DrawString(Text, Font, Brushes.Lime, 20, 1)
    End Sub
