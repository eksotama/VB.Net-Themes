...
Class YouTubeTabControl
    ...
    Protected Overrides Sub OnPaint(ByVal e As PaintEventArgs)
        ...
        For i = 0 To TabCount - 1
            ...
            If i = SelectedIndex Or i = intHoveredTab Then
                Dim intExtraWidth As Integer = 0
                If i > 0 Then intExtraWidth = 3
                G.FillRectangle(New SolidBrush(Color.FromArgb(153, 51, 0)), New Rectangle(TabTopBounds.X + 5, TabTopBounds.Y + TabTopBounds.Height - 4, TabTopBounds.Width - 15 + intExtraWidth, 3))

                'Draw text
                G.DrawString(TabPages(i).Text, Font, Brushes.Black, New Point(CInt(TabTopBounds.X + (TabTopBounds.Width / 2)) - 2, CInt(TabTopBounds.Y + (TabTopBounds.Height / 2)) - 1), New StringFormat With {.LineAlignment = StringAlignment.Center, .Alignment = StringAlignment.Center})
            Else
                G.DrawString(TabPages(i).Text, Font, Brushes.DimGray, New Point(CInt(TabTopBounds.X + (TabTopBounds.Width / 2)) - 2, CInt(TabTopBounds.Y + (TabTopBounds.Height / 2)) - 1), New StringFormat With {.LineAlignment = StringAlignment.Center, .Alignment = StringAlignment.Center})
            End If
        Next

        'Draw horizontal line under titles
        ...
        G.Dispose() : B.Dispose()
    End Sub
    Dim intHoveredTab As Integer = -1
    Protected Overrides Sub OnMouseMove(ByVal e As System.Windows.Forms.MouseEventArgs)
        Dim blnChangedHoverIndex As Boolean = False
        For i = 0 To TabCount - 1
            Dim TabTopBounds As New Rectangle(GetTabRect(i).X + 5, GetTabRect(i).Y + 2, GetTabRect(i).Width - 10, GetTabRect(i).Height - 4)
            If TabTopBounds.Contains(e.Location) Then
                intHoveredTab = i
                blnChangedHoverIndex = True
            End If
        Next
        If Not blnChangedHoverIndex Then
            intHoveredTab = -1
        Else
            '   Invalidate()
        End If
...
Class YouTubeButton
    ...
    Protected Overrides Sub PaintHook()
        ...
        Dim rctButtonContent As New Rectangle(0, 0, Width - 1, Height - 1)

        'Fill button
        Select Case State
            Case MouseState.Over
                'Fill button
                FillButton(rctButtonContent, Color.FromArgb(248, 248, 248), Color.FromArgb(238, 238, 238))
                'Draw border
                G.DrawPath(New Pen(Color.FromArgb(198, 198, 198)), CreateRound(rctButtonContent, 2))
                'Draw text
            ...
            Case MouseState.Down
                ...
                'Draw shadows
                'Left line
                Dim RCT_Left As New Rectangle(1, 1, 1, Height - 2)
                Dim LGB_Left_Line As New LinearGradientBrush(RCT_Left, Color.FromArgb(240, 240, 240), Color.FromArgb(230, 230, 230), 90)
                G.FillRectangle(LGB_Left_Line, RCT_Left)
                'Right line
                Dim RCT_Right As New Rectangle(Width - 2, 1, 1, Height - 2)
                Dim LGB_Right_Line As New LinearGradientBrush(RCT_Right, Color.FromArgb(240, 240, 240), Color.FromArgb(230, 230, 230), 90)
                G.FillRectangle(LGB_Right_Line, RCT_Right)
                'Top first line
            ...
            Case Else
                'Fill button
                FillButton(rctButtonContent, Color.FromArgb(252, 252, 252), Color.FromArgb(248, 248, 248))
                'Draw border
                G.DrawPath(New Pen(Color.FromArgb(211, 211, 211)), CreateRound(rctButtonContent, 2))
                'Draw text
    ...
    'Fill the button
    Sub FillButton(ByVal rctBounds As Rectangle, ByVal clrOne As Color, ByVal clrTwo As Color)
        Dim ButtonGradient As New LinearGradientBrush(rctBounds, clrOne, clrTwo, 90)
        G.FillPath(ButtonGradient, CreateRound(rctBounds, 2))
    End Sub
End Class

<DefaultEvent("TextChanged")> _
Class YouTubeTextBox
    Inherits ThemeControl154
    ...
    Sub New()
        ...
        AddHandler MyBase.GotFocus, AddressOf FocusBase
        AddHandler MyBase.LostFocus, AddressOf UnfocusBase
    ...
    Sub UnfocusBase(ByVal sender As Object, ByVal e As EventArgs)
        IsFocused = False
        Invalidate()
    End Sub

    Sub FocusBase(ByVal sender As Object, ByVal e As EventArgs)
        Base.Focus()
        If Base.Text = Watermark Then
            Base.Text = ""
        End If
    End Sub

    Dim IsFocused As Boolean = False
    Sub TextGotFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        IsFocused = True
    ...
    Sub TextLostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        IsFocused = False
    ...
    Protected Overrides Sub PaintHook()
        ...
        Dim rctBorder As New Rectangle(0, 0, Width - 1, Height - 1)

        'Draw shadow
        If IsFocused Then
            DrawShadowsFocused(rctBorder)
        Else
            DrawShadows(rctBorder)
        End If

        'Draw border
        If IsFocused Then
            G.DrawPath(New Pen(Color.FromArgb(185, 185, 185)), CreateRound(rctBorder, 2))
        Else
            G.DrawPath(New Pen(Color.FromArgb(204, 204, 204)), CreateRound(rctBorder, 2))
        End If

        If Not String.IsNullOrEmpty(Watermark) And IsFocused = False And Base.Text = "" Then Base.Text = Watermark : Base.ForeColor = Color.FromArgb(125, 125, 125)
    End Sub

    Sub DrawShadowsFocused(ByVal rctButtonBorder As Rectangle)
        'Vertical
    ...
    Sub DrawShadows(ByVal rctButtonBorder As Rectangle)
        'Vertical
...
Class YouTubeCheckbox
    ...
    Dim CheckButtonBorder As New Rectangle(0, 2, 13, 13)
    Protected Overrides Sub PaintHook()
        G.Clear(BackColor)

        G.SmoothingMode = SmoothingMode.HighQuality

        If _Checked Then DrawCheck()
        DrawBoxBorders(IsMouseHoveringOverBox())

        G.DrawString(Text, Font, TextBrush, New Point(17, 2))
    End Sub

    Private Function IsMouseHoveringOverBox() As Boolean
        Return (State = MouseState.Over Or State = MouseState.Down) And X <= CheckButtonBorder.X + CheckButtonBorder.Width
    End Function

    Private Sub DrawBoxBorders(ByVal blnHover As Boolean)
        If blnHover Then
            G.DrawPath(New Pen(Color.FromArgb(198, 198, 198)), CreateRound(CheckButtonBorder, 2))
        Else
            G.DrawPath(New Pen(Color.FromArgb(220, 220, 220)), CreateRound(CheckButtonBorder, 2))
        End If
    End Sub
        ...
        Try
            If e.Index >= 0 Then
                ...
                If InStr(e.State.ToString, "Selected,") > 0 Then
                    'Draw background
                    Dim LGB_SelectedItem As New LinearGradientBrush(ItemBounds, Color.FromArgb(175, 43, 38), Color.FromArgb(162, 40, 36), 90)
                    LGB_SelectedItem.SetBlendTriangularShape(0.5F, 1.0F)
                    e.Graphics.FillRectangle(LGB_SelectedItem, ItemBounds)
                    'Draw text
                    e.Graphics.DrawString(Items(e.Index).ToString(), Font, New SolidBrush(Color.FromArgb(246, 255, 255)), 5, CInt(e.Bounds.Y + (e.Bounds.Height / 2) - 7))
                Else
                    'Draw text
                    e.Graphics.DrawString(Items(e.Index).ToString(), Font, New SolidBrush(Color.FromArgb(85, 85, 85)), 5, CInt(e.Bounds.Y + (e.Bounds.Height / 2) - 7))
                End If
            End If
            'Draw borders
            CreateGraphics.DrawRectangle(New Pen(Color.FromArgb(226, 226, 226)), New Rectangle(0, 0, Width - 1, Height - 1))
            MyBase.OnDrawItem(e)
